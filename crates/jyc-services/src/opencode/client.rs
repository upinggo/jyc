            ...
            tokio::select! {
                        ...
                        Some(Ok(Event::Message(msg))) => {
                            ...
                            if event_type == "server.heartbeat" {
                                ...
                                if result.model_id.is_some() && !model_updated {
                                    if let Ok(mut ctx) = jyc_mcp::context::load_reply_context(directory).await {
                                        ...
                                        jyc_mcp::context::save_reply_context(directory, &ctx).await.ok();
                                    }
                                    model_updated = true;
                                }
                        }
                ...
                new_item = pending_rx.recv() => {
                    if let Some(item) = new_item {
                        ...
                        let storage = jyc_core::message_storage::MessageStorage::new(
                            ...
                            .await
                        {
                            ...
                            let cleaned_body = jyc_core::email_parser::strip_quoted_history(raw_body);
                        ...
                        if let Some(ref tokens_json) = part.tokens {
                            match serde_json::from_value::<TokenInfo>(tokens_json.clone()) {
                                Ok(token_info) => {
                                    ...
                                    crate::opencode::session::add_input_tokens(
