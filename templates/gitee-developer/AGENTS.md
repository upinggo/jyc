...
> If a clone fails, troubleshoot the issue (e.g., check network, Gitee API token) without
     ...
     - Run `{check_command}` and `{test_command}` to verify
      - Commit: 
        ...
        if [ "$(git branch --show-current)" = "main" ] || [ "$(git branch --show-current)" = "master" ]; then
          echo "FATAL: Refusing to commit on main/master branch. Use the git fetch+checkout pattern first."
          exit 1
        fi
